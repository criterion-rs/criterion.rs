...
#[derive(Debug, Default, Clone, Copy)]
pub struct InputAxisScale(pub AxisScale);

#[derive(Debug, Default, Clone, Copy)]
pub struct TimeAxisScale(pub AxisScale);

#[derive(Debug, Default, Clone, Copy)]
pub struct ThroughputAxisScale(pub AxisScale);

#[derive(Debug, Default, Clone)]
pub struct AxisScales {
    input: InputAxisScale,
    time: TimeAxisScale,
    throughput: ThroughputAxisScale,
}
...
impl PlotConfiguration {
    #[must_use]
    /// Set the axis scale ([linear] or [logarithmic]) for the input axis in summary plots.
    ///
    ...
    pub fn input_scale(mut self, new_scale: AxisScale) -> PlotConfiguration {
        self.axis_scale.input = InputAxisScale(new_scale);
        self
    }

    #[must_use]
    /// Set the axis scale for the time axis in summary plots.
    ///
    /// Defaults to [`AxisScale::Linear`].
    pub fn time_scale(mut self, new_scale: AxisScale) -> PlotConfiguration {
        self.axis_scale.time = TimeAxisScale(new_scale);
        self
    }

    #[must_use]
    /// Set the axis scale for the throughput axis in summary plots.
    ///
    /// Defaults to [`AxisScale::Linear`].
    pub fn throughput_scale(mut self, new_scale: AxisScale) -> PlotConfiguration {
        self.axis_scale.throughput = ThroughputAxisScale(new_scale);
        self
    }

    #[must_use]
    /// Set the axis scale ([linear] or [logarithmic]) for the all axis in summary plots.
    ///
    /// See [`PlotConfiguration::input_scale`] and [`PlotConfiguration::time_scale`] for details.
    /// Defaults to [`AxisScale::Linear`].
    ///
    /// [linear]: AxisScale::Linear
    /// [logarithmic]: AxisScale::Logarithmic
    pub fn summary_scale(mut self, new_scale: AxisScale) -> PlotConfiguration {
        self = self.input_scale(new_scale);
        self = self.time_scale(new_scale);
        self = self.throughput_scale(new_scale);
        self
    }
}
