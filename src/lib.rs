...
/// Axis range. Specified via [`PlotConfiguration::summary_range`].
#[derive(Debug, Default, Clone, Copy)]
pub enum AxisRange {
    /// Axis bounds are derived from the data being displayed ('zoom to fit')
    #[default]
    Fit,

    /// Same as [`AxisRange::Fit`], but the range is extended
    /// so that it always includes zero.
    FullScale,

    /// Axis range is autoscaled in an unspecified way
    Auto,
}
...
///     .summary_range(AxisRange::FullScale);
///
...
impl PlotConfiguration {
    ...
    #[must_use]
    /// Set the axis range ([zoom-to-fit], [full-scale] or [automatic])
    /// for the summary plots.
    ///
    /// Typically, you would want to only show the part of the axis
    /// that contains some graph data, but it may sometimes be useful
    /// to ensure that axis always contains zero point.
    ///
    /// [zoom-to-fit]: AxisRange::Fit
    /// [full-scale]: AxisRange::FullScale
    /// [automatic]: AxisScale::Auto
    pub fn summary_range(mut self, new_range: AxisRange) -> PlotConfiguration {
        self.summary_range = new_range;
        self
    }
}
